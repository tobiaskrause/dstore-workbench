...
void _get_file_that_not_exists(Broker& broker)
{
...
void _insert_duplicate(Broker& broker)
{
  std::string nodeName = "Node_1";
  std::shared_ptr<Node> node(new Node(nodeName));
  std::vector<std::shared_ptr<Node>> nodes = {node};
  ...
  //Insert again
  DataMapList dataMapList = broker.storeOnNodes(nodes, filename, *pTestData_1Stream);
  BOOST_CHECK(dataMapList.size() == 0);
  BOOST_CHECK(Node::Remove(*(node.get())));
}

void _distribute_with_SimpleBroker_one_node(Broker& broker)
{
  std::string nodeName = "Node_1";
  std::shared_ptr<Node> node(new Node(nodeName));
  std::vector<std::shared_ptr<Node>> nodes = {node};
...
void _distribute_with_SimpleBroker_two_nodes(Broker& broker)
{
